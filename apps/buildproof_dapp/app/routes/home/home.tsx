...
import { useGetTriplesWithPositionsQuery, configureClient } from '@0xintuition/graphql';
...
export function Home() {
    ...
    if (triplesData?.triples && Object.keys(ipfsDataMap).length > 0) {
      const newHackathons = triplesData.triples.map(triple => {
        ...
        return {
          ...
          tags,
          cashPrize: `$${cashPrize}`,
          imgSrc: ipfsData?.image || 'https://avatars.githubusercontent.com/u/186075312?s=200&v=4',
  ...
  const tagCounts = hackathons.flatMap(h => h.tags).reduce((acc, tag) => {
    acc[tag] = (acc[tag] || 0) + 1;
    return acc;
