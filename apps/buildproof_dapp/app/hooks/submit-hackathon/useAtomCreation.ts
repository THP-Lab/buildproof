...
export const useAtomCreation = () => {
  ...
  const mapAtomToRole = async (atomMapping: AtomMapping, ipfsHash: string, role: string) => {
  ...
  const createAtomMapping = async (atomsWithRoles: Array<{ ipfsHash: string; role: string }>): Promise<AtomMapping> => {
  ...
  const ensureAtomsExist = async (atomMapping: AtomMapping): Promise<AtomMapping> => {
