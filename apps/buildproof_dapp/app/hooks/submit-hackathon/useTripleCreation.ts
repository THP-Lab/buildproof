...
export const useTripleCreation = () => {
  ...
  const checkTripleExists = async (
    ...
    try {
      const exists = await publicClient.readContract({
        ...
        args: [BigInt(subject.toString()), BigInt(predicate.toString()), BigInt(object.toString())]
      }) as boolean;
  ...
  const createTriples = async (atomIds: { [key: string]: bigint }) => {
    ...
    const prizeTypes = ['first_place', 'second_place', 'third_place', 'other_place_a', 'other_place_b'];
